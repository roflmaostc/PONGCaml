...
let move_cpu (bar:bar) diff gp (ball:ball)= 
    let key =(if ball.x>bar.x+bar.width-bar.width/5 then 'd' else if ball.x<bar.x+bar.width/5 then 'a' else 'k')
    in
    if key = 'd' then
      if bar.x+bar.width+diff>gp.x_size then bar
      else {bar with x=bar.x+diff}
...
let rec game gp =
  ...
  let rec aux bar1 bar2 ball =
    let bar1 = move_bar bar1 10 gp 
    in
    let bar2 = move_cpu bar2 10 gp ball
    in
    let ball = move_ball ball 10 bar1 bar2 gp
    in
    update_gui gp bar1 bar2 ball;
    Unix.sleepf 0.01; aux bar1 bar2 ball
  in
  aux bar1 bar2 ball;;
